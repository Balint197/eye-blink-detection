...
class eye_blink_detector():
    ...
    def eye_blink(self, gray, rect, COUNTER, TOTAL):
        ...
        if ear < cfg.EYE_AR_THRESH:
            COUNTER += 1
            if (COUNTER >= cfg.EYE_AR_CONSEC_FRAMES_BLINK and COUNTER < cfg.EYE_AR_CONSEC_FRAMES_CLOSED):
                BLINK = 1
        ...
        # otherwise, the eye aspect ratio is not below the blink threshold
            ...
            CLOSED = 0
            COUNTER = 0
        return COUNTER, TOTAL, CLOSED, BLINK
...
def convert_rectangles2array(rectangles, image):
    ...
    for box in rectangles:
        [x0, y0, x1, y1] = max(0, box.left()), max(0, box.top()), min(
            box.right(), image.shape[1]), min(box.bottom(), image.shape[0])
...
def bounding_box(img, box, match_name=[]):
    ...
    if match_name[1] == "blink":
        color = (0, 255, 0)
    elif match_name[1] == "closed":
        color = (0, 0, 255)
    elif match_name[1] == "open":
        color = (255, 0, 0)
    ...
    for i in np.arange(len(box)):
        ...
        if not match_name:
            ...
            cv2.putText(img, match_name[i], (x0, y0-10),
